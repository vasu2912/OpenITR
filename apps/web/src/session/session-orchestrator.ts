...
const HEALTH_DISABILITY_DEDUCTION_AFFECTED_RESULT: AffectedResult = Object.freeze({
...
const OLD_REGIME_AFFECTED_RESULT: AffectedResult = Object.freeze({
...
const housePropertyCountOf = (facts: readonly ScopeFact[]): number => {
	const fact = facts.find(
		(candidate) => candidate.factKey === parseFactKey("scope.house-property-count"),
	);
	return fact?.state === "known" && fact.value.kind === "whole-number"
		? fact.value.value
		: 0;
};

const computeHousePropertyScenario = ({
	rulePack,
	analysisScopeFacts,
	answers,
}: Readonly<{
...
const HEALTH_DISABILITY_FACT_KEY_SET = new Set<FactKey>(
	Object.values(HEALTH_DISABILITY_DEDUCTION_FACT_KEYS),
);

const computeHealthDisabilityDeductionScenario = ({
	rulePack,
	answers,
	reconciliation,
}: Readonly<{
	...
	if (rulePack.taxConstants?.healthDisabilityDeductions === undefined) {
		return undefined;
	}
	const observedFactKeys = new Set(
		reconciliation.acceptedFacts.map((accepted) => accepted.factKey),
	...
	const observedFacts = reconciliation.acceptedFacts.flatMap(
		(accepted): readonly HealthDisabilityDeductionFact[] => {
			if (!HEALTH_DISABILITY_FACT_KEY_SET.has(accepted.factKey)) return [];
			return [
				{
					factKey: accepted.factKey,
					value: accepted.value,
...
const deriveSessionReview = (
	...
	const applicableResultIds = [
		...
		input.rulePack.taxConstants?.remainingDeductions !== undefined
			...
			: []),
		...(input.documentsStageEntered === true &&
		input.rulePack.taxConstants?.oldRegime !== undefined
			? [OLD_REGIME_AFFECTED_RESULT.resultId]
			: []),
...
const oldRegimeIssue = (issue: Readonly<{
	code: OldRegimeComputationIssue["code"];
...
const answerValue = (
	answers: readonly AttestedAnswerFact[],
	factKey: FactKey,
): AttestedAnswerFact["value"] | undefined =>
	answers.find((answer) => answer.factKey === factKey)?.value;

const acceptedRemainingMoney = (
	computation: Extract<RemainingDeductionComputation, { kind: "computed" }>,
	factKey: FactKey,
): ExactMoney => {
	const value = computation.facts.find(
		(fact) => fact.factKey === factKey,
	)?.value;
	if (typeof value !== "string") {
		throw new Error(`Completed remaining-deduction fact is missing: ${factKey}`);
	}
	return value;
};

const computeOldRegimeScenario = ({
	rulePack,
	factSetRevision,
	answers,
	salaryComputation,
	...
	const seniorCitizen = answerValue(
		answers,
		parseFactKey("taxpayer.senior-citizen"),
	);
	const superSeniorCitizen = answerValue(
		answers,
		parseFactKey("taxpayer.super-senior-citizen"),
	);
	if (typeof seniorCitizen !== "boolean") {
		issues.push({
			code: parseIssueCode("FACT_OLD_REGIME_AGE_CATEGORY_MISSING"),
			severity: "blocking",
			affectedFacts: [parseFactKey("taxpayer.senior-citizen")],
			recoveryAction: "Answer the cited resident age-category question.",
		});
	} else if (seniorCitizen && typeof superSeniorCitizen !== "boolean") {
		issues.push({
		...
		input: {
			...
			amounts: {
				...
				deductions: {
					...
					loanInterest: loanInterestDeductionComputation.oldRegimeTotal,
					donations: donationDeductionComputation.oldRegimeTotal,
					remaining: remainingDeductionComputation.oldRegimeTotal,
				},
			},
		},
	});
...
const regimeComparisonOf = ({
	...
	if (
		...
		estimateComputation?.kind !== "computed"
	) {
		return undefined;
	}
	return compareRegimes({
		oldRegime: oldRegimeComputation,
		newRegime: newRegimeComputation,
		taxesPaid: estimateComputation.summary.taxesPaid,
	});
};
...
const deriveSessionComputations = (
	...
	const healthDisabilityDeductionComputation =
	...
	const oldRegimeComputation = computeOldRegimeScenario({
		rulePack,
		factSetRevision,
		answers,
		salaryComputation: derived.salaryComputation,
	...
	return {
		...
		newRegimeComputation,
		oldRegimeComputation,
		regimeComparison: regimeComparisonOf({
			oldRegimeComputation,
			newRegimeComputation,
			estimateComputation,
		}),
...
> => {
	...
	const factSetRevision = factSetRevisionFor(generation);
	const housePropertyComputation = computeHousePropertyScenario({
		rulePack: context.rulePack,
		analysisScopeFacts,
		answers,
	...
	const shouldDefer =
		...
		context.regimeComparison?.kind === "computed" ||
	...
	return {
		...
		donationDeductionComputation,
		remainingDeductionComputation,
		newRegimeComputation: shouldDefer ? undefined : newRegimeComputation,
		oldRegimeComputation: shouldDefer ? undefined : oldRegimeComputation,
		regimeComparison: shouldDefer
			? undefined
			: regimeComparisonOf({
					oldRegimeComputation,
					newRegimeComputation,
...
> => {
	...
	const factSetRevision = factSetRevisionFor(context.recomputationGeneration);
	const estimateComputation =
...
> => {
	...
	const derived = deriveSessionComputations(
		input,
		factSetRevisionFor(generation),
	);
	const analysisScopeFacts = deriveItr1AnalysisScopeFacts({
		baseFacts: context.analysisScopeFacts,
		acceptedFacts: derived.reconciliation.acceptedFacts,
		attestedFacts: moneyAnswersOf(input.answers),
		rulePackId: context.rulePack.identity.id,
	});
	return {
		extractions,
				...
				on: {
					...
					"primary-regime-selected": {
						actions: sessionSetup.assign({
							primaryRegime: ({ context, event }) =>
								event.type === "primary-regime-selected"
									? event.selection
									: context.primaryRegime,
						}),
					},
					...
					"analysis-scope-question-answered": {
						actions: sessionSetup.assign(({ context, event }) => {
							...
							const analysisScopeFacts = deriveItr1AnalysisScopeFacts({
								...
								attestedFacts: moneyAnswersOf(answersOf(context.answerDecisions)),
								rulePackId: context.rulePack.identity.id,
							});
							const analysisScopeEvaluation = evaluateAnalysisScopeOf(
								{ rulePack: context.rulePack, facts: analysisScopeFacts },
							);
							const recomputationGeneration =
								context.recomputationGeneration + 1;
							if (analysisScopeEvaluation?.kind === "supported") {
								const derived = deriveSessionComputations(
									...
									factSetRevisionFor(recomputationGeneration),
								);
								return {
									analysisScopeFacts,
									analysisScopeEvaluation,
					...
					"missing-fact-answer-removed": {
						actions: sessionSetup.assign(({ context, event }) => {
							...
							const retainedAnswerIds = new Set(
								removeFactAnswerAndDependents({
									rulePack: context.rulePack,
									answers: answersOf(context.answerDecisions),
									answerId: event.decision.answer.answerId,
								}).map((answer) => answer.answerId),
							);
							const answerDecisions = context.answerDecisions.filter((decision) =>
								retainedAnswerIds.has(decision.answer.answerId),
							);
							return deriveDecisionComputations({
								context,
								answerDecisions,
			...
			switch (command.kind) {
				...
				case "answer-missing-fact-question": {
					...
					const attempt = evaluateFactAnswerAttempt({
						...
						applicableResultIds: [
							...new Set(
								snapshot.questionnaire.questions.map(
									(question) => question.affectedResult.resultId,
								),
							),
						],
